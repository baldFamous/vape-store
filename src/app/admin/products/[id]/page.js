...
export default async function EditProductPage({ params }) {
    // Await params first to satisfy Server Component rules for dynamic routes in Next.js 14+
    ...
    if (productId) {
        ...
        if (error) {
            errorMsg = "Product not found or database error.";
            console.error(error);
        } else {
            product = data;
        }
        ...
        <div className="flex min-h-screen overflow-hidden bg-surface">
            ...
            <main className="flex-1 ml-0 md:ml-64 relative overflow-y-auto custom-scrollbar h-screen">
                <header className="sticky top-0 w-full flex justify-between items-center px-8 py-6 bg-surface/80 backdrop-blur-xl z-40 border-b border-outline-variant/10">
                    <div>
                        <h1 className="text-3xl font-extrabold tracking-tight font-headline text-on-surface">Editar Producto</h1>
                    </div>
                ...
                <section className="px-8 py-10 flex justify-center">
                    <div className="w-full max-w-2xl">
                        {errorMsg ? (
                            <div className="p-6 bg-error/10 border border-error/50 rounded-xl text-error">
